...
test.describe('Cart Flow', () => {
  ...
  test('should show empty cart state when no items', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'ตะกร้าสินค้าว่างเปล่า' })).toBeVisible();
    await expect(page.getByRole('link', { name: 'จองต้นไม้' })).toBeVisible();
  });

  test('should show login warning in cart when not logged in', async ({ page }) => {
    ...
    const emptyCart = page.getByRole('heading', { name: 'ตะกร้าสินค้าว่างเปล่า' });
    await expect(emptyCart).toBeVisible();
  });

  test('shop page should show tree listings or no results', async ({ page }) => {
    await page.goto('/shop');
