...
test.describe("Authentication Flow", () => {
  test("should show login page correctly", async ({ page }) => {
    ...
    await expect(
      page.getByPlaceholder("0812345678 หรือ user@example.com"),
    ).toBeVisible();
  ...
  test("should show validation errors on empty submission", async ({
    ...
    const phoneInput = page.getByPlaceholder(
      "0812345678 หรือ user@example.com",
    );
    const isRequired = await phoneInput.evaluate(
