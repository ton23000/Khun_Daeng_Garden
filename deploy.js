...
async function deploy() {
  ...
  try {
    ...
    if (!fs.existsSync(localDir)) {
      throw new Error(`Build directory not found: ${localDir}. Please run 'npm run build' first.`);
    }
    ...
    try {
      ...
      console.log('No placeholder index.html found or error removing it (safe to ignore).');
    }
    ...
    console.log('Note: Next.js SSR apps require a Node.js server to run on the hosting.');
    ...
    console.log('IMPORTANT: You must SSH into the server and run `npm install` and `npm start` to run the Next.js app.');
  } catch (err) {
