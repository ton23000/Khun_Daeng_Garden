...
export default function BookingSuccessPage() {
  ...
  const downloadQRCode = () => {
    ...
    img.onload = () => {
      ...
      link.download = `QR-PromptPay-${booking.refCode || booking.id}.png`;
  ...
  return (
    <div
      className="container"
      style={{ padding: "4rem 1rem", maxWidth: "600px", textAlign: "center" }}
    >
      <div
        style={{
      ...
      <p
        style={{ color: "#0369a1", marginBottom: "2rem", fontSize: "0.875rem" }}
      >
        รหัสการจอง:{" "}
        <span style={{ fontFamily: "monospace", fontWeight: "bold" }}>
          {booking.refCode || booking.id}
        </span>
      </p>
      ...
      {["PENDING_APPROVAL", "PRE_ORDER"].includes(booking.status) ? (
        ...
        >
          ...
          <p style={{ marginBottom: "0.75rem", fontSize: "0.875rem" }}>
            คำสั่งซื้อของคุณอยู่ในคิวรอการอนุมัติจากทางร้าน
            เนื่องจากสินค้าบางรายการมี
            <br />
        ...
        >
          ...
          {/* PromptPay QR Code - locally generated */}
          <div
            style={{
          ...
          {booking.slipUrl &&
            (() => {
              let slipUrls: string[] = [];
              try {
                slipUrls = JSON.parse(booking.slipUrl);
              } catch {
                slipUrls = [booking.slipUrl];
              }
              return (
                <div
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    >
                      ✅ สลิปที่แนบ ({slipUrls.length} รูป)
                    </p>
                    <button
                  ...
                  <div
                    style={{
                      display: "grid",
                      gridTemplateColumns: `repeat(${Math.min(slipUrls.length, 3)}, 1fr)`,
                      gap: "0.5rem",
                    }}
                  >
                    {slipUrls.map((url, i) => (
      ...
      >
        ...
        {!["PENDING_APPROVAL", "PRE_ORDER"].includes(booking.status) && (
          <Button variant="primary" onClick={() => setIsModalOpen(true)}>
            {booking.slipUrl ? "🔄 แนบสลิปใหม่" : "แนบสลิปการโอนเงิน"}
          </Button>
        )}
      </div>
