    ...
    return (
        <>
            ...
            <div className="staff-layout">
                <div className={`staff-backdrop ${isSidebarOpen ? 'open' : ''}`} onClick={() => setIsSidebarOpen(false)}></div>
                ...
                <aside className={`staff-sidebar ${isSidebarOpen ? 'open' : ''}`}>
                    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '1.5rem 1rem' }}>
                        <div style={{ marginBottom: '1.5rem' }}>
                            <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#1d4ed8', margin: 0 }}>🧑‍💼 Staff Panel</h2>
                            {user && (
                                <p style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.25rem' }}>{user.firstName} {user.lastName}</p>
                            )}
                        </div>

                        <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1 }}>
                            <div style={{ fontSize: '0.7rem', fontWeight: 'bold', color: '#9ca3af', textTransform: 'uppercase', paddingLeft: '0.5rem', marginBottom: '0.25rem' }}>
                                จัดการออเดอร์
                            </div>
                            <Link href="/staff/orders" style={navStyle(pathname === '/staff/orders')}>
                                <span style={{ fontSize: '1.1rem' }}>🛒</span> ออเดอร์ทั้งหมด
                            </Link>
                            <Link href="/staff/reviews" style={navStyle(pathname === '/staff/reviews')}>
                                <span style={{ fontSize: '1.1rem' }}>⭐</span> จัดการรีวิว
                            </Link>
                        </nav>

                        <div style={{ marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid #bfdbfe' }}>
                            <Button
                                variant="outline"
                                fullWidth
                                onClick={logout}
                ...
                <div className="staff-main-wrapper">
                    <div className="staff-top-bar">
                        <button
                            ...
                            style={{ padding: '0.5rem', marginRight: '1rem', backgroundColor: 'white', border: '1px solid #bfdbfe', borderRadius: '0.375rem', display: 'flex', alignItems: 'center', cursor: 'pointer' }}
                        >
                            <svg width="20" height="20" fill="none" stroke="#1d4ed8" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path>
                            </svg>
                        </button>
                        <h1 style={{ fontSize: '1.25rem', fontWeight: 600, color: '#1d4ed8', margin: 0 }}>พนักงาน (Staff)</h1>
                    </div>
