...
export default function StaffOrdersPage() {
  ...
  const [trees, setTrees] = useState<
    { id: string; stock: number; reserved: number; [key: string]: unknown }[]
  >([]);
  ...
  const filterAndSortBookings = useCallback(() => {
    ...
    if (viewMode === "by-customer" && selectedCustomer) {
      result = result.filter(
        (b) => `${b.user.firstName} ${b.user.lastName}` === selectedCustomer,
      );
    }
    if (statusFilter) {
      if (statusFilter === "VERIFYING_PAYMENT") {
        result = result.filter(
          (b) => b.status === "VERIFYING_PAYMENT" || b.status === "PAID",
        );
      } else if (statusFilter === "PENDING_APPROVAL") {
        result = result.filter(
          (b) => b.status === "PENDING_APPROVAL" || b.status === "PRE_ORDER",
        );
      } else {
  ...
  const handleDelete = async (id: string, refCode: string) => {
    if (
      !confirm(
        `คุณแน่ใจหรือไม่ที่จะลบออเดอร์ ${refCode}?\n\nการลบจะไม่สามารถกู้คืนได้`,
      )
    )
      return;
    try {
  ...
  const customers = Array.from(
    new Set(bookings.map((b) => `${b.user.firstName} ${b.user.lastName}`)),
  ).sort();

  const getCustomerStats = (customerName: string) => {
    const cb = bookings.filter(
      (b) => `${b.user.firstName} ${b.user.lastName}` === customerName,
    );
    return {
      totalOrders: cb.length,
    ...
    <div>
      ...
      >
        {[
          {
            ...
            count: bookings.filter(
              (b) =>
                b.status === "PENDING_APPROVAL" || b.status === "PRE_ORDER",
            ).length,
          },
          ...
          <Card
            ...
            style={{
              ...
              borderColor: statusFilter === stat.id ? stat.color : undefined,
            }}
            ...
            <CardContent style={{ padding: "1rem", textAlign: "center" }}>
              ...
              >
                {stat.count !== undefined
                  ? stat.count
                  : bookings.filter((b) => b.status === stat.id).length}
              </p>
            </CardContent>
          </Card>
        ))}
        <Card
          style={{
            borderColor:
              trees.filter(
                (t) => t.stock - t.reserved > 0 && t.stock - t.reserved < 5,
              ).length > 0
                ? "#f59e0b"
                : "#e5e7eb",
            cursor: "pointer",
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            <Button
              variant={
                viewMode === "all" && !statusFilter ? "primary" : "outline"
              }
              onClick={() => {
                setViewMode("all");
                setSelectedCustomer(null);
              ...
              style={{
                ...
                justifyContent: "center",
                backgroundColor:
                  viewMode === "all" && !statusFilter ? "#1d4ed8" : undefined,
                borderColor: "#1d4ed8",
                color:
                  viewMode === "all" && !statusFilter ? "white" : "#1d4ed8",
              }}
            >
              ทั้งหมด
            </Button>
            <Button
              variant={
                statusFilter === "PENDING_APPROVAL" ? "primary" : "outline"
              }
              onClick={() => handleStatusClick("PENDING_APPROVAL")}
              style={{
                backgroundColor:
                  statusFilter === "PENDING_APPROVAL" ? "#f97316" : undefined,
                borderColor: "#f97316",
                color:
                  statusFilter === "PENDING_APPROVAL" ? "white" : "#f97316",
                minWidth: "130px",
                display: "flex",
                justifyContent: "center",
              }}
            ...
            <Button
              ...
              style={{
                ...
                justifyContent: "center",
                borderColor: "#1d4ed8",
                color: viewMode === "by-customer" ? "white" : "#1d4ed8",
                backgroundColor:
                  viewMode === "by-customer" ? "#1d4ed8" : undefined,
              }}
            >
              แยกตามลูกค้า
            </Button>
                    ...
                    style={{
                      ...
                      border:
                        selectedCustomer === customer
                          ...
                          : "1px solid #d1d5db",
                      backgroundColor:
                        selectedCustomer === customer ? "#dbeafe" : "white",
                      cursor: "pointer",
                      fontSize: "0.875rem",
      ...
      <style
        dangerouslySetInnerHTML={{
          __html: `.desktop-table-view { display: none; } .mobile-card-view { display: flex; flex-direction: column; gap: 1rem; padding-bottom: 2rem; } @media (min-width: 768px) { .desktop-table-view { display: block; } .mobile-card-view { display: none; } }`,
        }}
      />
          ...
          filteredBookings.map((booking) => (
            ...
            >
              ...
              >
                ...
                >
                  {["PENDING_APPROVAL", "PRE_ORDER"].includes(
                    booking.status,
                  ) ? (
                    <div style={{ display: "flex", gap: "0.5rem" }}>
                      <Button
                        size="sm"
                        onClick={async () => {
                          if (!confirm(`อนุมัติออเดอร์ #${booking.refCode}?`))
                            return;
                          try {
                            const res = await fetch(
                              `/api/admin/bookings/${booking.id}/action`,
                              {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={async () => {
                          if (!confirm(`ปฏิเสธออเดอร์ #${booking.refCode}?`))
                            return;
                          try {
                            const res = await fetch(
                              `/api/admin/bookings/${booking.id}/action`,
      ...
      <div className="desktop-table-view">
        <div style={{ overflowX: "auto", paddingBottom: "1rem" }}>
          <Card style={{ minWidth: "1100px", border: "1px solid #e5e7eb" }}>
                        ...
                        >
                          {editingId === booking.id ? (
                            <ThaiDatePicker
                              ...
                              onChange={(val) =>
                                setEditForm({ ...editForm, pickupDate: val })
                              }
                            />
                          ) : (
                            ...
                            >
                              <Button
                                ...
                                onClick={async () => {
                                  if (
                                    !confirm(
                                      `อนุมัติออเดอร์ #${booking.refCode}?`,
                                    )
                                  )
                                    return;
                                  try {
      ...
      <SlipViewer
        ...
        slipUrls={(() => {
          try {
            const p = JSON.parse(viewingSlip || "[]");
            return Array.isArray(p) ? p : [viewingSlip!];
          } catch {
            return viewingSlip ? [viewingSlip] : [];
          }
        })()}
        startIndex={0}
