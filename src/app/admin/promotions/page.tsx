...
export default function AdminPromotionsPage() {
  ...
  const fetchTrees = async () => {
    try {
      const res = await fetch(`/api/trees?t=${Date.now()}`, {
        cache: "no-store",
      });
      if (res.ok) {
  ...
  const categories = Array.from(
    new Set(
      trees.flatMap((t) =>
        t.category ? t.category.split(",").filter(Boolean) : [],
      ),
    ),
  ).sort();
  ...
  const filteredTrees = trees.filter((t) => {
    ...
    const matchCategory =
      categoryFilter === "all" ||
      (t.category &&
        t.category.split(",").filter(Boolean).includes(categoryFilter));
  ...
  const openSingleEdit = (tree: Tree) => {
    ...
    setDiscountValue(
      tree.isPromotion && tree.originalPrice
        ? Math.round((1 - tree.price / tree.originalPrice) * 100)
        : 0,
    );
    setPromoName(tree.promotionName || "");
    setPromoEndDate(
      tree.promotionEndDate ? tree.promotionEndDate.split("T")[0] : "",
    );
    setShowModal(true);
  };
    ...
    try {
      for (const id of treeIds) {
        ...
        if (discountType === "percent") {
          newPrice = Math.round(
            currentOriginalPrice * (1 - discountValue / 100),
          );
        } else {
    ...
    <div>
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          {selected.size > 0 && (
            <>
              ...
              <Button
                onClick={handleBulkRemovePromotion}
                variant="outline"
            ...
            <option key={cat} value={cat}>
              {cat} (
              {
                trees.filter(
                  (t) =>
                    t.category &&
                    t.category.split(",").filter(Boolean).includes(cat),
                ).length
              }
              )
            </option>
      ...
      >
        ...
        >
          ...
          >
            {trees.filter((t) => t.isPromotion).length}
          </div>
      ...
      <div className="mobile-card-view">
        ...
        >
          <input
            type="checkbox"
            checked={
              selected.size === filteredTrees.length && filteredTrees.length > 0
            }
            onChange={toggleSelectAll}
            id="selectAllMobile"
          />
          <label htmlFor="selectAllMobile" style={{ fontWeight: 500 }}>
            เลือกทั้งหมด
          </label>
        </div>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "0.875rem", color: "#6b7280" }}>
                    ...
                    <span
                      style={{
                        color: tree.isPromotion ? "#166534" : "#9ca3af",
                      }}
                    >
                  ...
                  >
                    <span
                      style={{
                        fontWeight: "bold",
                        color: tree.isPromotion ? "#dc2626" : "#1f2937",
                      }}
                    >
                ...
                >
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={
                        selected.size === filteredTrees.length &&
                ...
                {filteredTrees.map((tree) => {
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        borderBottom: "1px solid #e5e7eb",
                        backgroundColor: selected.has(tree.id)
                          ? "#fef3c7"
                          : "transparent",
                      }}
                    >
                      ...
                      <td
                        style={{
                          ...
                          textAlign: "right",
                          color: "#6b7280",
                          textDecoration: tree.isPromotion
                            ? "line-through"
                            : "none",
                        }}
                      >
                      ...
                      <td style={{ padding: "0.75rem", textAlign: "center" }}>
                        <span
                          style={{
                            ...
                            height: "10px",
                            borderRadius: "50%",
                            backgroundColor: tree.isPromotion
                              ? "#16a34a"
                              : "#d1d5db",
                            marginRight: "0.25rem",
                          }}
          ...
          >
            <div style={{ padding: "2rem 2rem 0", overflowY: "auto", flex: 1 }}>
              ...
              >
                {isBulk
                  ? `🏷️ ตั้งโปรโมชัน ${selected.size} รายการ`
                  : `🏷️ ตั้งโปรโมชัน: ${editingTree?.name}`}
              </h2>
                      ...
                      style={{
                        ...
                        border:
                          discountType === "percent"
                            ...
                            : "1px solid #d1d5db",
                        backgroundColor:
                          discountType === "percent" ? "#fee2e2" : "white",
                        color:
                          discountType === "percent" ? "#dc2626" : "#374151",
                        fontWeight: "bold",
                        cursor: "pointer",
                      }}
                    >
                      ...
                      style={{
                        ...
                        border:
                          discountType === "fixed"
                            ...
                            : "1px solid #d1d5db",
                        backgroundColor:
                          discountType === "fixed" ? "#fee2e2" : "white",
                ...
                <div>
                  ...
                  <div style={{ position: "relative" }}>
                    <input
                      ...
                      placeholder={
                        discountType === "percent" ? "เช่น 20" : "เช่น 250"
                      }
                      style={{
                        width: "100%",
                        padding: "0.75rem",
                    ...
                    >
                      {discountType === "percent" ? (
                        <>
                          ราคาเดิม ฿
                          {(
                            editingTree.originalPrice || editingTree.price
                          ).toLocaleString()}{" "}
                          →{" "}
