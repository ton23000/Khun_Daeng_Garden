...
export default function TreesSoldPage() {
  ...
  const totalTreesSold = Object.values(treeCounts).reduce(
    (sum, tree) => sum + tree.count,
    0,
  );
  const uniqueTreeTypes = Object.keys(treeCounts).length;
  const topTrees = Object.values(treeCounts)
          ...
          <CardContent>
            ...
            >
              {filteredBookings.length > 0
                ? (totalTreesSold / filteredBookings.length).toFixed(1)
                : 0}
            </div>
            <p
              style={{
