...
export default function OrdersOverviewPage() {
  ...
  const statusCounts = {
    PENDING_APPROVAL: filteredBookings.filter(
      (b) => b.status === "PENDING_APPROVAL" || b.status === "PRE_ORDER",
    ).length,
    PENDING: filteredBookings.filter((b) => b.status === "PENDING").length,
    VERIFYING_PAYMENT: filteredBookings.filter(
  ...
  const activeOrders =
    ...
    statusCounts.VERIFYING_PAYMENT +
                  ...
                  style={{
                    ...
                    border:
                      dateFilter === filter
                        ...
                        : "1px solid #d1d5db",
                    backgroundColor:
                      dateFilter === filter ? "#dcfce7" : "white",
      ...
      >
        ...
        <Card style={{ borderLeft: "4px solid #dc2626" }}>
          ...
          <CardContent>
            ...
            >
              {((cancelledOrders / totalOrders) * 100 || 0).toFixed(1)}%
              ของทั้งหมด
            </p>
          </CardContent>
        </Card>
