...
export default function SalesOverviewPage() {
  ...
  const totalFullPayments = confirmedBookings
    .filter((b) => b.status === "COMPLETED")
    .reduce((sum, b) => sum + b.totalPrice, 0);
  const pendingSales = confirmedBookings
    .filter((b) => ["PREPARING", "READY", "CONFIRMED"].includes(b.status))
    .reduce((sum, b) => sum + (b.totalPrice - b.deposit), 0);

  const averageOrderValue =
    confirmedBookings.length > 0 ? totalSales / confirmedBookings.length : 0;
    ...
    <div>
      ...
      >
        <Card style={{ borderLeft: "4px solid #166534" }}>
          ...
          <CardContent>
            <div
              style={{ fontSize: "2rem", fontWeight: "bold", color: "#166534" }}
            >
          ...
          <CardContent>
            ...
            >
              ...
              <div>
                ...
                >
                  <span style={{ fontSize: "0.875rem", color: "#6b7280" }}>
                    ออเดอร์ที่ยืนยันแล้ว
                  </span>
                  <span style={{ fontWeight: "bold" }}>
                    {confirmedBookings.length} รายการ
                  </span>
                </div>
              </div>
              <div>
                ...
                >
                  ...
                  <span style={{ fontWeight: "bold", color: "#3b82f6" }}>
                    {((totalDeposits / totalSales) * 100 || 0).toFixed(1)}%
                  </span>
                </div>
              </div>
            </div>
