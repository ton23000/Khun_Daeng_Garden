...
export default function ReportsPage() {
  ...
  const totalSales = confirmedBookings.reduce(
    (sum, b) => sum + (b.status === "COMPLETED" ? b.totalPrice : b.deposit),
    0,
  );

  const totalDeposits = confirmedBookings.reduce(
    (sum, b) => sum + b.deposit,
    0,
  );

  const totalTreesSold = filteredBookings.reduce(
    (sum, b) => sum + b.items.reduce((acc, item) => acc + item.quantity, 0),
    0,
  );
  ...
  const getFilterLabel = () => {
    switch (dateFilter) {
      case "day":
        return `วันที่ ${formatThaiDate(selectedDate)}`;
      case "month":
        return `เดือน ${formatThaiDate(selectedDate, { month: "long", year: "numeric" })}`;
      case "year":
        return `ปี ${formatThaiDate(selectedDate, { year: "numeric" })}`;
      default:
        return "ทั้งหมด";
    }
  };
  ...
  const downloadReport = () => {
    ...
    // ---- ดาวน์โหลด ----
    const filterStr =
      dateFilter === "all" ? "ทั้งหมด" : selectedDate.substring(0, 10);
        ...
        >
          ...
          <CardContent>
            ...
            <p style={{ fontSize: "0.75rem", color: "#6b7280" }}>
              เงินมัดจำ: ฿{totalDeposits.toLocaleString()}
            </p>
            <p
              style={{
                fontSize: "0.75rem",
