...
export default function AdminTreesPage() {
  ...
  useEffect(() => {
    if (formData.isPromotion && formData.originalPrice > 0) {
      ...
      if (discountType === "percent") {
        newPrice = Math.round(
          formData.originalPrice * (1 - (discountValue || 0) / 100),
        );
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(
          editingTree
            ? "แก้ไขข้อมูลต้นไม้เรียบร้อยแล้ว"
            : "เพิ่มต้นไม้ใหม่เรียบร้อยแล้ว",
        );
        setIsModalOpen(false);
        setEditingTree(null);
        resetForm();
        ...
        const errorData = await res.json();
        alert(
          `เกิดข้อผิดพลาด: ${errorData.error || "ไม่สามารถบันทึกข้อมูลได้"}`,
        );
      }
    } catch (error) {
  ...
  const openEdit = (tree: Tree) => {
    ...
    setFormData({
      ...
      promotionEndDate: tree.promotionEndDate
        ? new Date(tree.promotionEndDate).toISOString().split("T")[0]
        : "",
      stock: tree.stock || 0,
    });
  ...
  return (
    <div>
      ...
      >
        <div style={{ flex: 1 }}>
          <SearchBar
            placeholder="รหัสสินค้า, ชื่อต้นไม้, ประเภท, tags..."
                ...
                filteredTrees.map((tree) => (
                  ...
                  >
                    <td style={{ padding: "1rem" }}>
                      <img
                        src={tree.images[0] || "/placeholder-tree.svg"}
                        alt={tree.name}
                        style={{
                          width: "50px",
                          height: "50px",
                    ...
                    <td style={{ padding: "1rem" }}>
                      ...
                      >
                        ...
                        {tree.tags?.flatMap((t) => t.split(",")).filter(Boolean)
                          .length > 3 && (
                          <span
      ...
      {isModalOpen && (
        ...
        >
          ...
          >
            <CardHeader>
              <CardTitle>
                {editingTree ? "แก้ไขต้นไม้" : "เพิ่มต้นไม้ใหม่"}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <form
              ...
              >
                ...
                >
                  <Input
                    label={
                      formData.isPromotion ? "ราคาปกติ (บาท)" : "ราคา (บาท)"
                    }
                    type="number"
                    value={
                      formData.isPromotion
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-medium">
                    รูปภาพ ({formData.images.length} รูป)
                  </label>
                  <label
                    style={{
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setFormData((prev) => ({
                            ...prev,
                            isPromotion: checked,
                            originalPrice: checked
                              ? prev.originalPrice || prev.price
                              : prev.originalPrice,
                          }));
                        }}
                        style={{
                    ...
                    >
                          ...
                          {discountType === "percent" ? (
                            ...
                            <>
                              ...
                              {formData.originalPrice > 0
                                ? Math.round(
                                    (1 -
                                      discountValue / formData.originalPrice) *
                                      100,
                                  )
                                : 0}
                              %)
                      ...
                      <div>
                        ...
                        <ThaiDatePicker
                          ...
                          onChange={(val) =>
                            setFormData({ ...formData, promotionEndDate: val })
                          }
                        />
                      </div>
                    </div>
                ...
                >
                  {editingTree ? (
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => handleDelete(editingTree.id)}
      ...
      {isCategoryModalOpen && (
        ...
        >
          ...
          >
            <CardHeader>
              <CardTitle>เพิ่มประเภทใหม่</CardTitle>
            </CardHeader>
            <CardContent>
              <form
              ...
              >
                <Input
                  ...
                  placeholder="เช่น 🌞 ไม้แดด, 🌤️ ไม้รำไร"
                  required
                />
                <div
                  style={{
