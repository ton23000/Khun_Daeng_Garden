  ...
  try {
    ...
    for (const item of validated.items) {
      ...
      if (tree.stock < item.quantity) {
        isPreOrder = true;
        console.log(
          "[Booking API] Pre-order detected for tree:",
          item.treeName,
        );
      }
    }
    console.log(
      "[Booking API] Stock check complete. Is pre-order:",
      isPreOrder,
    );
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const newBooking = await tx.booking.create({
        data: {
          ...
          pickupDate: validated.items[0]?.pickupDate
            ? new Date(validated.items[0].pickupDate).toISOString()
    ...
    console.log(
      "[Booking API] Booking created and stock reserved successfully:",
      booking.id,
    );
    ...
    try {
      ...
      console.log(
        "[Booking API] Admin notification created for booking:",
        booking.refCode,
      );
    } catch (notifError) {
      console.error(
        "[Booking API] Failed to create admin notification:",
        notifError,
      );
    ...
    if (process.env.NODE_ENV === "production" && userExists.email) {
      try {
        ...
        console.log(
          "[Booking API] Confirmation email sent to:",
          userExists.email,
        );
      } catch (emailError) {
        console.error(
          "[Booking API] Failed to send confirmation email:",
          emailError,
        );
