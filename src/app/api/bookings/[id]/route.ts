...
export async function PUT(
  ...
  try {
    ...
    if (validated.pickupDate)
      updateData.pickupDate = new Date(validated.pickupDate);
    ...
    if (
      ...
      for (const item of bookingItems) {
        ...
        if (tree) {
          ...
          console.log(
            `[Booking Update] Order completed - Tree ${item.treeId}: -${decrementAmount} stock, +${item.quantity} sold, -${reserveDecrement} reserved`,
          );
        }
      }
    }
    ...
    if (
      ...
      currentBooking.user &&
      booking.userId &&
      validated.status !== currentBooking.status
    ) {
      console.log(
        "🔔 [PUT] Status changed from",
...
export async function PATCH(
  ...
  try {
    ...
    if (
      ...
      for (const item of bookingItems) {
        ...
        if (tree) {
          ...
          console.log(
            `[Booking PATCH] Order completed - Tree ${item.treeId}: -${decrementAmount} stock, +${item.quantity} sold, -${reserveDecrement} reserved`,
          );
        }
      }
    }
    ...
    if (validated.status && currentBooking.user && booking.userId) {
      console.log(
        "🔔 Checking notification trigger for status:",
        validated.status,
      );
      let message = "";
      let type = "info";
...
export async function DELETE(
  ...
  try {
    ...
    if (
      !["PENDING_APPROVAL", "CANCELLED", "COMPLETED"].includes(booking.status)
    ) {
