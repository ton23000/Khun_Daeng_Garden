  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Only PENDING_APPROVAL and PRE_ORDER bookings can be approved/rejected",
        },
        { status: 400 },
      );
    }
    ...
    if (action === "approve") {
      ...
      for (const item of booking.items) {
        ...
        if (available < item.quantity) {
          console.warn(
            `[Admin Action] Warning: Approving booking with insufficient stock for ${item.tree.name} (Available: ${available}, Required: ${item.quantity})`,
          );
      ...
      // Stock is already reserved at checkout time (in api/bookings/route.ts)
      // so we don't need to increment reserved stock again here.
      console.log(
        "[Admin Action] Booking approved - Stock was already reserved at creation",
      );
      ...
      console.log(
        `[Admin Action] Approved booking ${bookingId}, reserved stock`,
      );
      return NextResponse.json({
        success: true,
